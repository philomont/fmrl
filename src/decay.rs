...
/// Render one tile into 128×128×4 RGBA bytes.
pub fn render_tile(tile: &TileData, age: &AgeEntry, palette: &Palette, now_ms: u64) -> Vec<u8> {
    ...
    render_tile_with_fade(tile, age, palette, base_fade)
}
...
pub fn render_tile_with_policy(
    ...
    render_tile_with_fade(tile, age, palette, fade)
}

fn render_tile_with_fade(tile: &TileData, age: &AgeEntry, palette: &Palette, fade: f32) -> Vec<u8> {
    ...
    for i in 0..pixel_count {
        ...
        if idx != PAPER_INDEX as usize && is_stroke_edge(&indices, i) {
...
fn is_stroke_edge(indices: &[u8], pixel_idx: usize) -> bool {
